...
export const ProfilePage: React.FC = () => {
  ...
  const { user, logoutUser, updateUserPIN } = useAuthStore();
  ...
  const handleProfileSave = (e: React.FormEvent) => {
    e.preventDefault();
    setEditSuccess('Citizen registry database updated successfully.');
  ...
  const handlePINReset = (e: React.FormEvent) => {
    ...
    // Attempt Reset
    const success = updateUserPIN(oldPIN, newPIN);
    if (success) {
            ...
            <form onSubmit={handleProfileSave} className="flex flex-col gap-4">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div className="flex flex-col gap-1.5">
                  ...
                  <div className="relative">
                    <MapPin size={14} className="absolute left-3.5 top-1/2 -translate-y-1/2 text-[var(--text-secondary)]" />
                    <input
                      type="text"
                      value={address}
